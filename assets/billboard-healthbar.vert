...
void main() 
{
	...
	vec4 vertexPosition_worldspace = model * vec4(0.0, 0.0, 0.0, 1.0);
	gl_Position = projection * view * vertexPosition_worldspace;
	gl_Position /= gl_Position.w;
	
	float offsetx = (0.5 - uv.x) * sizex;
	float offsety = (0.5 - uv.y) * sizey;

	gl_Position.xy -= vec2(offsetx, offsety);
