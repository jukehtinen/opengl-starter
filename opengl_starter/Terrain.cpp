...
namespace opengl_starter
{
        ...
        : _tesselated(tesselated), _shader(shader), _texHeight(texHeight), _texColor(texColor), _texColor2(texColor2)
    {
        ...
        {
            float tx = static_cast<float>(i % resolution);
            float ty = static_cast<float>(i / resolution);
        ...
        _elements = static_cast<GLuint>(indices.size());
    ...
    void Terrain::Render(const glm::mat4& projection, const glm::mat4& view, const glm::vec3& eye)
    {
        ...
        if (_tesselated)
        {
            ...
            glProgramUniform1f(_shader->teseProg, glGetUniformLocation(_shader->teseProg, "displacement"), 8.0f);
